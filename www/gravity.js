let _s = new CSSStyleSheet();
//_s.insertRule("body{margin:0;overflow:hidden;user-select:none;}");
document.adoptedStyleSheets.push(_s);

// All scripts extracted from https://mrdoob.com/projects/chromeexperiments/google-gravity/
...
window.addEventListener("load", () => {
  ...
  b2Settings.b2_velocityThreshold =
    (1.0 * b2Settings.b2_lengthUnitsPerMeter) /
  ...
  b2Math.b2MulMM = function (A, B) {
    var C = new b2Mat22(
      0,
      b2Math.b2MulMV(A, B.col1),
      b2Math.b2MulMV(A, B.col2),
    );
    return C;
  };
  ...
  b2BroadPhase.prototype = {
    ...
    MoveProxy: function (proxyId, aabb) {
      ...
      if (
        proxyId == b2Pair.b2_nullProxy ||
        b2Settings.b2_maxProxies <= proxyId
      ) {
        return;
      }
    ...
    TestOverlapValidate: function (p1, p2) {
      for (var axis = 0; axis < 2; ++axis) {
        var bounds = this.m_bounds[axis];
        if (
          bounds[p1.lowerBounds[axis]].value >
          bounds[p2.upperBounds[axis]].value
        )
          return false;
        if (
          bounds[p1.upperBounds[axis]].value <
          bounds[p2.lowerBounds[axis]].value
        )
          return false;
      }
      return true;
    ...
    Query: function (
      ...
      var lowerQuery = b2BroadPhase.BinarySearch(
        bounds,
        boundCount,
        lowerValue,
      );
      var upperQuery = b2BroadPhase.BinarySearch(
        bounds,
        boundCount,
        upperValue,
      );
  ...
  b2Collision.FindMaxSeparation = function (
    ...
    for (var i = 0; i < count1; ++i) {
      var dot =
        poly1.m_normals[i].x * dLocal1X + poly1.m_normals[i].y * dLocal1Y;
      if (dot > maxDot) {
        maxDot = dot;
        edge = i;
      }
  ...
  b2Collision.FindIncidentEdge = function (c, poly1, edge1, poly2) {
    ...
    normal1Local2Y =
      normal1Local2X * tMat.col2.x + normal1Local2Y * tMat.col2.y;
    normal1Local2X = tX;
    var vertex21 = 0;
    var vertex22 = 0;
    var minDot = Number.MAX_VALUE;
    for (var i = 0; i < count2; ++i) {
      ...
      var dot =
        normal2Local2X * normal1Local2X + normal2Local2Y * normal1Local2Y;
      if (dot < minDot) {
        minDot = dot;
        vertex21 = i1;
        vertex22 = i2;
  ...
  Object.extend(b2CircleShape.prototype, {
    ...
    Synchronize: function (position1, R1, position2, R2) {
      ...
      var p1X =
        position1.x +
        (R1.col1.x * this.m_localPosition.x +
          R1.col2.x * this.m_localPosition.y);
      var p1Y =
        position1.y +
        (R1.col1.y * this.m_localPosition.x +
          R1.col2.y * this.m_localPosition.y);
  ...
  Object.extend(b2PolyShape.prototype, {
    ...
    Synchronize: function (position1, R1, position2, R2) {
      ...
      this.m_position.x =
        this.m_body.m_position.x +
        (R2.col1.x * this.m_localCentroid.x +
          R2.col2.x * this.m_localCentroid.y);
      this.m_position.y =
        this.m_body.m_position.y +
        (R2.col1.y * this.m_localCentroid.x +
          R2.col2.y * this.m_localCentroid.y);
      if (this.m_proxyId == b2Pair.b2_nullProxy) {
        return;
      }
      var hX;
    ...
    Support: function (dX, dY, out) {
      ...
      for (var i = 1; i < this.m_vertexCount; ++i) {
        var value =
          this.m_coreVertices[i].x * dLocalX +
          this.m_coreVertices[i].y * dLocalY;
        if (value > bestValue) {
          bestIndex = i;
          bestValue = value;
        }
  ...
  b2Body.prototype = {
    ...
    GetWorldPoint: function (localPoint) {
      return b2Math.AddVV(
        this.m_position,
        b2Math.b2MulMV(this.m_R, localPoint),
      );
    },
  ...
  b2World.prototype = {
    ...
    CreateJoint: function (def) {
      ...
      if (def.collideConnected == false) {
        var b =
          def.body1.m_shapeCount < def.body2.m_shapeCount
            ? def.body1
            : def.body2;
    ...
    PrepareVelocitySolver: function () {
      ...
      if (this.m_revolute2) {
        ...
        K +=
          this.m_ratio *
          this.m_ratio *
          (b2.m_invMass + b2.m_invI * crug * crug);
      }
      this.m_mass = 1.0 / K;
      b1.m_linearVelocity.x +=
        b1.m_invMass * this.m_impulse * this.m_J.linear1.x;
      b1.m_linearVelocity.y +=
        b1.m_invMass * this.m_impulse * this.m_J.linear1.y;
      b1.m_angularVelocity += b1.m_invI * this.m_impulse * this.m_J.angular1;
      b2.m_linearVelocity.x +=
        b2.m_invMass * this.m_impulse * this.m_J.linear2.x;
      b2.m_linearVelocity.y +=
        b2.m_invMass * this.m_impulse * this.m_J.linear2.y;
  ...
  Object.extend(b2PrismaticJoint.prototype, {
    ...
    GetReactionForce: function (invTimeStep) {
      ...
      var ax1X =
        tImp *
        (tMat.col1.x * this.m_localXAxis1.x +
          tMat.col2.x * this.m_localXAxis1.y);
      var ax1Y =
        tImp *
        (tMat.col1.y * this.m_localXAxis1.x +
          tMat.col2.y * this.m_localXAxis1.y);
      var ay1X =
        tImp *
        (tMat.col1.x * this.m_localYAxis1.x +
          tMat.col2.x * this.m_localYAxis1.y);
      var ay1Y =
        tImp *
        (tMat.col1.y * this.m_localYAxis1.x +
          tMat.col2.y * this.m_localYAxis1.y);
  ...
  Object.extend(b2PulleyJoint.prototype, {
    ...
    SolveVelocityConstraints: function (step) {
      ...
      if (this.m_limitState1 == b2Joint.e_atUpperLimit) {
        ...
        this.m_limitImpulse1 = b2Math.b2Max(
          0.0,
      ...
      if (this.m_limitState2 == b2Joint.e_atUpperLimit) {
        ...
        this.m_limitImpulse2 = b2Math.b2Max(
          0.0,
  ...
  function run() {
    ...
    requestAnimationFrame(() => {
      loop();
      _s.insertRule("*{padding:0 !important;margin:0 !important;}");
    });
  }
