...
clip_icon.addEventListener("click", () => {
  navigator.clipboard.writeText(sha_element.innerHTML).then(() => {
    alert(`Copied ${sha_element.innerHTML}`);
  });
});
clip_icon.innerHTML =
...
sha_element.parentElement!.parentElement!.appendChild(clip_icon);
