...
storemode.add_argument(
    "-r", help="Restore from a file generated by -d. This is prioritized over -s"
)
...
for k, v in data.items():
    if v:
        _data[k] = v
if not _data and not args.d and not args.r:
...
for token in tokens:
    try:
        ...
        if _data or (args.r and store[userid]):
            response = requests.patch(
                "https://discord.com/api/v9/users/@me/settings",
                json=_data | (store.get(userid, {}) if args.r else {}),
                headers=headers,
            )
