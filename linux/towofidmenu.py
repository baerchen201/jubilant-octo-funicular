...
argparser.add_argument("-c", help="reads a display name from stdin and returns the associated command")
argparser.add_argument("-C", help="use command as prefix if not explicitly specified", action="store_true")
    ...
    for item in f.read().decode().strip().splitlines():
        ...
        match len(line):
            ...
            case 2: # Command and display name, no prefix*
                command = line[1]
                display = line[0]
                if args.C: prefix = command.split()[0] # If -C argument used, use command as prefix
            case 3: # Command, display name and prefix specified, assign values
                command = line[2]
                display = line[0]
                prefix = line[1]
        if prefix: display = f"[{prefix}] {display}" # Adjust display name if prefix present
...
raise SystemExit(int(bool(args.c))) # Smart usage of int(bool(x)), if -c argument specified no match was found, so exit code 1, otherwise 0
